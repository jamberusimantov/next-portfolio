...
const handler = async (req: NextApiRequest, res: NextApiResponse<response>) => {
  ...
  try {
    switch (req.method) {
      ...
      case 'POST': {
        if (req.body) {
          const response = await DBer.POST(Messages, 'message', mockMessage, body)
      ...
      case 'GET': {
        const response = await DBer.GET(Messages, 'message', mockMessage, req.query)
        if (response.error) throw response.error;
        return res.status(200).json(response);
      }
      ...
      case 'DELETE': {
        if (req.query.id) {
          const response = await DBer.DELETE(Messages, 'message', mockMessage, id)
          if (response.error) throw response.error;
          return res.status(200).json(response);
        }
      }
      ...
      case 'PUT': {
        if (req.query.id && req.body) {
          ...
          const response = await DBer.PUT(Messages, 'message', mockMessage, body, id)
          if (response.error) throw response.error;
          return res.status(200).json(response);
        }
      }
